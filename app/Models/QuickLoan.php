...
class QuickLoan extends Model implements MemberLoan
{
    use IsMemberLoan;
    ...
    {
        static::creating(function (QuickLoan $loan): void {
            if (blank($loan->loan_type)) {
                $loan->loan_type = LoanTypes::QUICK;
            }

            if (blank($loan->loan_period_months)) {
                $loan->loan_period_months = 1;
            }
        });
    }
